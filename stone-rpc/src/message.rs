...
use rmpv::{
    decode::{self, value},
    encode, Integer, Utf8String, Value,
};
        ...
        let notification = if let Value::Array(items) = &array[2] {
            if let Value::Integer(notf_type) = items[0] {
                match notf_type.as_u64() {
                    Some(RESPONSE_MESSAGE_SUCCESS) => MeResult::Success(items[1].clone()),
                    Some(RESPONSE_MESSAGE_FAILURE) => MeResult::Failure(items[1].to_string()),
                    _ => return Err(DecodeError::Invalid),
                }
            } else {
                return Err(DecodeError::Invalid);
